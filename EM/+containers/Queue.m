classdef Queue < containers.ArrayList
    ...
    methods
        ...
        function elementOrElements = Readqueue(obj,HowMany)
            %% 读取队列。
            % 有时候，并不需要真的从队列中弹出数据，而是读一下即可。
            assert(obj.Count-HowMany >= 0, 'Cannot read queue more elements than are currently in the queue.');
            elementOrElements = obj.elementAt(1:HowMany);
        end
